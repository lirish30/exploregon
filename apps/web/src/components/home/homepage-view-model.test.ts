...
test('buildHomepageViewModel falls back to placeholders when seeded content is incomplete', () => {
  ...
  assert.equal(result.utilitySnapshot.metrics.length, 3)
  assert.equal(result.utilitySnapshot.metrics[0]?.label, 'Tide table')
  assert.equal(result.utilitySnapshot.metrics[0]?.value, 'High at 2:14 PM')
